...
<!-- MAIN -->
<main>
    <h1 class="title">Archive Medicine</h1>
    <ul class="breadcrumbs">
        <li><a href="#">Admin</a></li>
        <li class="divider">/</li>
        <li><a href="#" class="active">Archive Medicine</a></li>
    </ul>
    ...
    <div class="container mt-3 table-border">
        ...
        <table class="table table-hover"  id="admin_table">
            ...
            <tbody>
                <?php
                $sql = "SELECT * FROM tbl_archivemedical";
                    ...
                    if ($count > 0) {
                        while ($rows = mysqli_fetch_assoc($result)) {
                            ...
                            <tr>
                                ...
                                <td><img src="medical_image/<?php echo $image ?>" style="width: 70px;"></td>
                                <td><?php echo $created_at; ?></td>

                                <td>
                                <button type="button" class="btn btn-primary " value="<?php echo $id; ?>">Restored Details</button>
                                <form action="code.php" method="post">
                                        <button type="button" class="btn-del delete_medicalbtn" value="<?= $id; ?>">Delete Permanent</button>
                                    </form>
                                </td>
                            </tr>
                            <?php
...
<script>
    ...
    button.addEventListener('click', function() {
        ...
        if (confirm("Are you sure you want to restore this medical?")) {
            fetch(`restore_medical.php?id=${medicalId}`, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert("Failed to restore the medical.");
                }
            })
            .catch(error => {
                console.error('Error:', error);
                alert("An error occurred while restoring the medical.");
            });
        }
    });
});
